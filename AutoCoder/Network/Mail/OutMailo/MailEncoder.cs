...
/*******************************************************************************/
	...
	internal class MailEncoder
	{
		...
		/// <returns></returns>
		internal static string ConvertHeaderToQP(
			string InValue, string InCharSet, bool InForce )
		{
			bool doConvert = InForce ;
			if ( doConvert == false ) 
			{
				int Lx = InValue.Length ;
				for ( int Ix =0 ; Ix < Lx ; Ix++ ) 
				{
					if (( InValue[Ix] > 126 ) || ( InValue[Ix] < 32 )) 
					{
						doConvert = true ;
						break;
					}
			...
			{
				string result =
					"=?" + InCharSet + "?Q?" + ConvertToQP( InValue, InCharSet ) + "?=" ;
				return result ;
			}
			else
				return InValue ;
