  ...
  public class EditableComboBox : ComboBox
  {
    ...
    void ApplyTextValueToItemsSource()
    {
      if (this.ApplyEntryToItemsSource == true)
      {
        ...
        {
          var collection = this.ItemsSource as IList<string>;

          // items source is not a list of strings.
          if (collection == null)
            Debug.WriteLine("EditableComboBox. Cannot apply entry text " 
              + this.TextValue 
              + " ItemsSource is not a list of strings.");
