...
//        nonWord = rv.Item3;
    ...
    // ------------------------------------- IsolateWordText -------------------------
    public static Tuple<TextLocation, TextWord, AtomText> IsolateWordText(
      ScanStream ScanStream,
      TextTraits Traits,
      ...
      int Bx, int? NonWordBx)
    {
      ...
      if (LiteralType == null)
      {
        ...
        wordEx = new StreamLocation(Bx + lx - 1).ToTextLocation(ScanStream);
        wordPart = new TextWord(
          ScanStream.Substring(Bx, lx), WordClassification.Identifier, Traits);
        string userCode = null;
        atomText = new AtomText(ScanAtomCode.Identifier,
          ScanStream.Stream.Substring(Bx, lx), null,
          wordBx, wordEx, userCode);
      }
      ...
      {
        wordBx = new StreamLocation(Bx).ToTextLocation(ScanStream);
        int lx = LitText.Length;
        wordEx = new StreamLocation(Bx + lx - 1).ToTextLocation(ScanStream);
        wordPart = new TextWord(LitText, LiteralType.Value, Traits);

        atomText = new AtomText(
          LiteralType.Value.ToScanAtomCode( ),
         ScanStream.Stream.Substring(Bx, lx),
         LiteralType.Value,
          wordBx, wordEx);
      }

      return new Tuple<TextLocation, TextWord, AtomText>(wordBx, wordPart, atomText);
    }
      ...
      string InString, int InIx, int InLx, ScanPatterns InPatterns)
    {
      ...
      while (true)
      {
        ...
        // scan forward in the text for any of the LeadChars of the patterns being
        // scanned for.
        ...
        // found a lead char. Now match for the entire pattern of each pattern with
        // the found lead char.
        var rv = InPatterns.MatchPatternsAtStringLocation(InString, scr.ResultPos, ex);
        pat = rv.Item1;
        matchLx = rv.Item2;
        if (pat != null)
        {
    ...
    public static PatternScanResults 
      ScanEqualAny(string Text, int Start, ScanPatterns Patterns)
    {
      ScanPattern pat = null;
      int matchLx = 0;
      int ix = Start;
      List<MatchScanPattern> equalPats = null;
      PatternScanResults nonWord = null;
      ...
      while (true)
      {
        ...
        ix = scr.ResultPos;
        var rv = Patterns.MatchPatternsAtStringLocation(Text, ix, Text.Length - 1);
        pat = rv.Item1;
        matchLx = rv.Item2;
        equalPats = rv.Item3;
        nonWord = rv.Item4;
      ...
      string InString, Whitespace InWhitespace, int InMaxReturnChars)
    {
      ...
      {
        string fsChars = InString.SubstringLenient(ix, InMaxReturnChars ) ;
      ...
      string Text, ScanPatterns Patterns, int Start)
    {
      ...
      while (true)
      {
        ...
        if (fx == -1)
          break;
        ScanPattern pat = Patterns.ScanPatternsArray[fx];

        bool anyIsMatch = false;
        int maxMatchLx = 0;
        int remLx = ex - ix + 1;
        while (pat != null)
        {
          ...
          // is a match. update length of longest match.
          if (isMatch == true)
          {
            anyIsMatch = true;
            if (matchLx > maxMatchLx)
              maxMatchLx = matchLx;
          }
    ...
    public static Tuple<ScanPattern, int, int, List<MatchScanPattern>> ScanReverseEqual(
      string Text, int Start, ScanPatterns Patterns)
    {
      ...
      while (true)
      {
        ...
        var rv = Patterns.MatchPatternsAtStringLocation(Text, ix, Start);
        pat = rv.Item1;
        matchLx = rv.Item2;
        equalPats = rv.Item3;

        // ScanReverse pattern found.
        if (pat != null)
        {
          break;
        }
      ...
      return new
        Tuple<ScanPattern, int, int, List<MatchScanPattern>>(pat, ix, matchLx, equalPats);
    }
    ...
    /// <summary>
    /// Scan reverse until non of the patterns are found at location in string.
    /// ( Note: this method only works for patterns that are 1 char in length )
    /// </summary>
      ...
      string Text, int Start, ScanPatterns Patterns)
    {
      ...
      // the max length of the patterns to scan for.
      ...
      while (true)
      {
        ...
        var rv = Patterns.MatchPatternsAtStringLocation(Text, ix, endIx);
        var pat = rv.Item1;
        matchLx = rv.Item2;
        equalPats = rv.Item3;

        // pattern not found at the pos in string.
        if (pat == null)
        {
          foundIx = ix;
          break;
