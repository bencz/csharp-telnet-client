...
namespace AutoCoder.Text
{
  ...
  //    
    ...
    /// <returns></returns>
    public static char AsciiHexPairToChar(this string InText, int InStart)
    {
      string hexPair = InText.Substring(InStart, 2);
      byte b1 = System.Convert.ToByte(hexPair, 16);
      char[] ch1 = System.Text.Encoding.ASCII.GetChars(new byte[] { b1 });
      return ch1[0];
    }
    ...
    public static string ByteToHex(
      byte[] InBytes, int InStart, int InLength,
      char? SepChar = null)
    {
      ...
      for (int ix = 0; ix < InLength; ++ix)
      {
        ...
        string s1 = ByteToHex(InBytes[InStart + ix]);
        sb.Append(s1);
      }
    ...
    /// <returns></returns>
    public static string CapitalizeFirstChar(string InValue)
    {
      string s1 = Head(InValue, 1).ToUpper();
      if (InValue.Length < 2)
        return s1;
      else
        return s1 + InValue.Substring(1);
    }
    ...
    /// <param name="InWord"></param>
    /// <returns></returns>
    public static string CapitalizeWord(string InWord)
    {
      if ((InWord == null) || ( InWord.Length == 0 ))
        return null;
      else if (InWord.Length == 1)
        return System.Globalization.CultureInfo.CurrentCulture.TextInfo.ToUpper(InWord);
      else
      {
        string lcWord =
        System.Globalization.CultureInfo.CurrentCulture.TextInfo.ToLower(InWord);
        char ucChar = System.Globalization.CultureInfo.CurrentCulture.TextInfo.ToUpper(
          InWord[0] );
...
#if skip
    /// <summary>
    /// test if the string is not null and not zero length.
    /// </summary>
    /// <param name="Text"></param>
    /// <returns></returns>
    public static bool ContainsText(this string Text)
    {
      if (Text == null)
        return false;
    ...
    private static string MaskedDecrypt_Decrypt(string InEncrypted, string InMask)
    {
      ...
      string mask = Repeat(InMask, InEncrypted.Length);
    ...
    private static string MaskedEncrypt_Encrypt(string InPlain, string InMask)
    {
      ...
      string mask = Repeat(InMask, InPlain.Length);
    ...
    /// <returns></returns>
    public static string Repeat(string InPattern, int InLength)
    {
      StringBuilder sb = new StringBuilder(InLength);
      while (true)
      {
        if (sb.Length >= InLength)
          break;
        sb.Append(InPattern);
      }
    ...
    /// <returns></returns>
    public static string ReplaceAll( string InString, char InPattern, char InReplace )
    {
    ...
    public static string ShortAbbreviateName(string InName)
    {
      ...
      if (sb.Length == 0)
      {
        sb.Append(Stringer.SubstringLenient(InName, 0, 3));
      }
    ...
    {
      if (Stringer.IsBlank(InValue) == true)
        return 0;
      else
    ...
    public static DateTime? TryParseDateTime(string InText)
    {
      DateTime vlu;
    ...
    public static Int32? TryParseInt32(this string InText)
    {
      Int32 vlu;
      bool rv = Int32.TryParse(InText, out vlu);
      if (rv == false)
        return null;
      else
        return vlu;
    ...
    /// <summary>
    /// TryParse a substring as an integer value.
    /// </summary>
    /// <param name="InText"></param>
    /// <param name="InStart"></param>
    /// <param name="InLength"></param>
    /// <returns></returns>
    public static int? TryParseInt32(this string InText, int InStart, int InLength)
    {
      int intValue = 0;
